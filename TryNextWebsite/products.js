    ...
    filteredProducts = allProducts.filter(product => {
        ...
        if (searchQuery) {
            const searchFields = [
                ...
                product.name_bn ? product.name_bn.toLowerCase() : '',
                product.description_bn ? product.description_bn.toLowerCase() : ''
            ];
...
function applyFilters() {
    ...
    // Apply price filter
    if (priceFilter) {
        const [min, max] = priceFilter.split('-').map(p => p.replace('+', '').replace('৳', ''));
    ...
    displayedProducts = 12; // Show more products on dedicated page
    renderProducts();
}
